...
export const PoolBalance = {
  encode(message: PoolBalance, writer: Writer = Writer.create()): Writer {
    if (message.index !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): PoolBalance {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...basePoolBalance } as PoolBalance;
  ...
  fromPartial(object: DeepPartial<PoolBalance>): PoolBalance {
    const message = { ...basePoolBalance } as PoolBalance;
    if (object.index !== undefined && object.index !== null) {
